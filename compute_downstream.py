...
def main(args):
    ...
    if len(temp_list)>0:
        raise ValueError('Following columns not in adata.obs.columns: %s'%','.join(temp__list))
    else:
    ...
    score_file_list = [x for x in os.listdir(score_dir) 
    ...
    if len(VARIABLE_LIST)>0: 
        STR_ANALYSIS+='\n%d. Variable-disease correlation analysis for (%s)'%(i,','.join(VARIABLE_LIST))
    ...
    for trait in dic_score.keys():
        ...
        control_list = [x for x in dic_score[trait].columns if x.startswith('ctrl_norm_score')]
        n_ctrl = len(control_list)
        df_reg = adata.obs.loc[cell_list, CELLTYPE_LIST+VARIABLE_LIST].copy()
        df_reg = df_reg.join(dic_score[trait].loc[cell_list, ['norm_score']+control_list])        
        
        # Cell type-disease analysis: association+heterogeneity
        for ct_col in CELLTYPE_LIST:
            ct_list = sorted(set(adata.obs[ct_col]))
            col_list = ['n_cell','n_ctrl','assoc_mcp','assoc_mcz','hetero_mcp','hetero_mcz']
            ...
            for ct in ct_list:
                ...
                v_ctrl_score_q95 = np.quantile(df_reg.loc[ct_cell_list, control_list], 0.95, axis=0)
                mc_p = ((v_ctrl_score_q95>=score_q95).sum()+1) / (v_ctrl_score_q95.shape[0]+1)
            ...
            df_res.to_csv(os.path.join(OUT_FOLDER,'%s.scdrs_ct.%s'%(trait, ct_col.replace(' ','_'))),
                          sep='\t', index=True)
            print('%s: cell type-level analysis with label=%s (sys_time=%0.1fs)'
                  %(trait, ct_col, time.time()-sys_start_time))
            
        # Variable-disease correlation
        if len(VARIABLE_LIST)>0:
            col_list = ['n_ctrl','corr_mcp','corr_mcz']
            df_res = pd.DataFrame(index=VARIABLE_LIST, columns=col_list, dtype=np.float32)
            for var_col in VARIABLE_LIST:
                corr_ = np.corrcoef(df_reg[var_col], df_reg['norm_score'])[0,1]
                v_corr_ = [np.corrcoef(df_reg[var_col], df_reg['ctrl_norm_score_%d'%x])[0,1] 
            ...
            df_res.to_csv(os.path.join(OUT_FOLDER,'%s.scdrs_var'%trait), sep='\t', index=True)
            print('%s: cell-level variable-disease correlation analysis (sys_time=%0.1fs)'
        ...
        if FLAG_GENE is True:
            ...
            df_res.to_csv(os.path.join(OUT_FOLDER,'%s.scdrs_gene'%trait), sep='\t', index=True)
            print('%s: disease gene prioritization (sys_time=%0.1fs)'
                  %(trait, time.time()-sys_start_time))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cell_type', type=str, required=False, default=None,
    ...
    parser.add_argument('--cell_variable', type=str, required=False, default=None,
                        help='Comma-seprated coloumn names for cell-level variables, '
                        'used for associating cell-level variables to disease scores') 
    parser.add_argument('--flag_gene', type=str, required=False, default=False,
