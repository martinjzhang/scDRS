...
def main(args):
    ...
    if MATCH_OPT not in ['mean', 'mean_var']:
        raise ValueError('# MATCH_OPT needs to be one of [mean, mean_var]')
    ...
    if COV_FILE is not None:
        ...
        print('# COV_FILE loaded, cov_list=%s'%cov_list)
        ...
        adata.X = md._reg_out(adata.X, adata.obs[cov_list].values)
        adata.X += adata.var['mean']
        print('# Covariates regressed out from adata, time=%0.1f'%(time.time()-sys_start_time))
    ...
    for trait in df_gs.index:
        ...
        if len(gene_list)<10:
            ...
            continue
        df_res = md.score_cell(adata, gene_list, ctrl_match_key=MATCH_OPT, n_ctrl=N_CTRL, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--match_opt', type=str, required=False, default='mean')
