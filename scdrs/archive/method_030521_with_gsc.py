...
def score_cell(data, 
    ...
    dic_ctrl_list,dic_ctrl_weight = _select_ctrl_geneset(df_gene, gene_list, gene_weight,
                                                         ...
                                                         random_seed, verbose)    
    # Compute raw scores        
    v_raw_score,v_score_weight = _compute_raw_score(adata, gene_list, gene_weight, weight_opt, cov_list=cov_list)
    ...
    for i_ctrl in range(n_ctrl): 
        mat_ctrl_raw_score[:,i_ctrl],mat_ctrl_weight[:,i_ctrl] = \
            _compute_raw_score(adata, dic_ctrl_list[i_ctrl], dic_ctrl_weight[i_ctrl], weight_opt, cov_list=cov_list)
                
    # Compute normalized scores 
    v_var_ratio_c2t = np.zeros(n_ctrl)
    for i_ctrl in range(n_ctrl):
