...
def score_cell(data, 
               ...
               random_seed=0,
               verbose=False,
               ctrlgene_list=None, # remove
               ctrl_opt='mean_match'  # remove
              ):
    ...
    ----
        ...
        ctrl_match_key : str
            The variable based on which control genes are selected. 
            Should be in the column of data.obs
        suffix : str
...
def _select_ctrl_geneset(input_df_gene, gene_list, gene_weight,
    ...
    if ctrl_opt=='mean_match':
        ...
        df_gene['qbin'] = pd.qcut(df_gene['mean'], q=n_genebin, labels=False, duplicates='drop')
